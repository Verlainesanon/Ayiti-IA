...
def evaluate_model(
    ...
    """
    from src.inference.chat import AyitiChat
    ...
    # Créer l'interface de chat temporaire avec le modèle chargé
    # On génère directement pour éviter un double chargement
    model.eval()
    chat = AyitiChat.__new__(AyitiChat)
    chat.model = model
    chat.tokenizer = tokenizer
    chat.device = str(device)
    ...
    metrics["perplexity"] = compute_perplexity(
        model, tokenizer, texts_for_ppl, device=device
    )
